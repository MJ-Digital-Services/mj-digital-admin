  ...
  const ToolbarButton = ({
    ...
    <button
      ...
      className={`p-1.5 rounded transition-colors ${
        active ? "bg-red-100 text-red-700" : "text-slate-600 hover:bg-slate-200 hover:text-slate-900"
      }`}
    >
      {children}
    </button>
    ...
    `}</style>
      <div className="border border-slate-200 rounded-lg overflow-hidden bg-white">
        ...
        <div className="flex flex-wrap items-center gap-0.5 p-2 border-b border-slate-200 bg-slate-50 sticky top-0 z-10">
          ...
          {/* Image (only if upload handler provided) */}
          {onImageUpload && (
            <>
