...
export default function NewsPage() {
  ...
  const { data, isLoading } = useQuery({
    ...
    queryFn: async () => {
      const res = await newsApi.getAll(queryParams);
    ...
    <div>
      ...
      <div className="p-6 space-y-5">
        ...
        <NewsTable
          news={news}
          isLoading={isLoading}
          onEdit={(item) => router.push(`/news/${item._id}/edit`)}
          onDelete={(item) => { setDeleteTarget(item); setConfirmOpen(true); }}
        />

        {pagination.totalPages > 1 && (
          <div className="flex items-center justify-between text-sm text-slate-600">
            <span>{pagination.total} total articles</span>
